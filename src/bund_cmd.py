...
class BUND_GEN(object):
    def __init__(self, _desc, _epilog):
        ...
        try:
            ...
            self.BUND_HOME = "/tmp"
        try:
            self.BUND_DEFAULT_ENV = get_from_env("BUND_DEFAULT_ENV", default="default")
        except:
            self.BUND_DEFAULT_ENV = "default"
        self.BUND_ARGS = get_from_env("BUND_ARGS", default=None)
        self.parser = argparse.ArgumentParser(prog='bund', description=_desc, epilog=self.epilog)
        self.parser.add_argument("--banner", action="store_true", help="Display banner during start")
        self.parser.add_argument("--home", "-H", type=str, default=self.BUND_HOME,
                                 help="Path to the default home directory or URL")
        self.parser.add_argument("--default_environment", "-D", type=str, default=self.BUND_DEFAULT_ENV,
    ...
    def preflight(self):
        ...
        if self.env.ready != True:
            self.ready = False
            return False
        self.env.cfg["BUND_HOME"] = self.args.home
        self.env.cfg["BUND_VERBOSE"] = self.args.v
        self.env.cfg["BUND_ENV_NAME"] = self.args.default_environment
        self.env.cfg["BUND_ENV_PATH"] = "%s/%s/"%(self.env.cfg["BUND_HOME"], self.env.cfg["BUND_ENV_NAME"])
        if self.env.cfg["BUND_ENV_PATH"] == "+":
            self.env.cfg["BUND_ENV_PATH"] = "+" + posixpath.abspath(self.env.cfg["BUND_ENV_PATH"][1:])
        elif self.env.cfg["BUND_ENV_PATH"] != "@":
            self.env.cfg["BUND_ENV_PATH"] = "+" + posixpath.abspath(self.env.cfg["BUND_ENV_PATH"])
        else:
            pass
        if not check_reference_read(self.env.cfg["BUND_ENV_PATH"], True):
            self.warning("Configuration HOME %s can not be accessed"%self.env.cfg["BUND_ENV_PATH"])
        else:
            self.ok("Configuration HOME set to %s"%self.env.cfg["BUND_ENV_PATH"])
        return True
    def process(self):
        self.args = self.parser.parse_args()
        self.ok("Initializing (bund) shell")
        if not self.parse_and_set_args():
            return False
        self._call_hiera("make_doc", "Error creating documentation in %s")
        self.env = BUND_ENV(self)
